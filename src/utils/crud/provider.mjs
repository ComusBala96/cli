...
import { createFile, getFile, getFirstString, getPath, updateFile } from '../functions/functions.mjs';

export function updateCrudProvider(res) {
    let path = getPath(res, 'provider');
    let name = getFirstString(res?.namespace) + 'RepositoryServiceProvider';
    let ext = php;
